...
//(s)hms_coll_flag: leaf called T: (s)hms_collimator_cut_flag
//calculate Kf & Pf
//what is up with missing mass?
//double check lists
...
void usecooks()
{
	...
	else if (user_runtype == "SRC")
	{
		if (user_target == "D2")
		{
			inROOT = new TFile("../../../../../../../../volatile/hallc/c-cafe-2022/OFFLINE/PASS1/CAFE_OUTPUT/skimmed_pass1/d2_SRC_skim_total.root", "READ");
		}
		else if (user_target == "Be9")
		{
	...
	TH2F* H2_cthrq_pmiss = Histo::mk_cthrq_pmiss(inputtree, "H2_cthrq_pmiss", SAccList, binning);
	...
	TH2F* H2_W_pmiss = Histo::mk_W_pmiss(inputtree, "H2_W_pmiss", SAccList, binning);

	TH2F* H2_thrq_pmiss_pid = Histo::mk_thrq_pmiss(inputtree, "H2_thrq_pmiss_pid", SAccList, binning);
	TH2F* H2_cthrq_pmiss_pid = Histo::mk_cthrq_pmiss(inputtree, "H2_cthrq_pmiss_pid", SAccList, binning);
	...
	TH2F* H2_W_pmiss_pid = Histo::mk_W_pmiss(inputtree, "H2_W_pmiss_pid", SAccList, binning);

	TH2F* H2_thrq_pmiss_pid_acc = Histo::mk_thrq_pmiss(inputtree, "H2_thrq_pmiss_pid_acc", SAccList, binning);
	TH2F* H2_cthrq_pmiss_pid_acc = Histo::mk_cthrq_pmiss(inputtree, "H2_cthrq_pmiss_pid_acc", SAccList, binning);
	...
	TH2F* H2_W_pmiss_pid_acc = Histo::mk_W_pmiss(inputtree, "H2_W_pmiss_pid_acc", SAccList, binning);

	TH2F* H2_thrq_pmiss_pid_acc_kin = Histo::mk_thrq_pmiss(inputtree, "H2_thrq_pmiss_pid_acc_kin", SAccList, binning);
	TH2F* H2_cthrq_pmiss_pid_acc_kin = Histo::mk_cthrq_pmiss(inputtree, "H2_cthrq_pmiss_pid_acc_kin", SAccList, binning);
	...
	TH2F* H2_W_pmiss_pid_acc_kin = Histo::mk_W_pmiss(inputtree, "H2_W_pmiss_pid_acc_kin", SAccList, binning);
	...
	int max_loops = 100000;					//Define maximum number of events to process
	Long64_t nentries;
	...
	for (int i = 0; i < nentries; i++)
	{
		...
		accp_hms = Cuts::Accp_HMS(hdelta, hxptar, hyptar, hYColl, hXColl);
		accp_shms = Cuts::Accp_SHMS(edelta, exptar, eyptar, eYColl, eXColl);
		...
		{
			kin = Cuts::kinMF(Q2, pmiss, emiss_nuc, emiss_nuc, (th_rq / dtr), target);
		}
		else
		{
		...
		//----- SHMS -----
		if (pid_hms && pid_shms && epctime && noedtm)
		{
		...
		if (pid_hms && pid_shms && accp_hms && accp_shms && accp_z && epctime && noedtm)
		{
		...
		if (pid_hms && pid_shms && accp_hms && accp_shms && accp_z && kin && epctime && noedtm)
		{
